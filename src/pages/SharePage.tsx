...
const SharePage = () => {
  ...
  const dayCourse = shareData?.courseDetails.filter((detail) => detail.day === currentDay);
  ...
  return (
    <>
      ...
      <PageTransformWrapper>
        ...
        >
          {shareData?.courseDetails && shareData?.courseDetails.length > 0 && (
            <Ticket.Wrapper>
              <Ticket.Header
                noZoom
                label="열정적인 활동가에게 추천하는"
                title={shareData.name || 'title'}
                description={shareData.courseDescription || 'description'}
                coordinates={shareData?.courseDetails.map((toursite) => ({
                  lat: toursite.latitude,
                  lng: toursite.longitude,
                }))}
              />
              <Ticket.Bottom images={shareData?.courseDetails.map((site) => site.photoUrl)} />
            </Ticket.Wrapper>
          )}
          <Stack
            flexDirection={'row'}
          ...
          >
            {mockArray(shareData?.duration || 0).map((_, index) => {
              return (
                <ToggleButton
                  key={index}
        ...
        <Stack sx={{ padding: '8px 14px 120px 14px' }} gap="30px">
          ...
          <Button
            ...
            sx={{ height: '45px' }}
            onClick={() => {
              onShare(Number(shareData?.groupId));
            }}
          >
